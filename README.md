...
- **설문 조사**: 사용자의 피부 타입, 고민, 선호하는 스타일을 분석합니다.
- **데이터 기반 매칭**: 분석된 데이터를 바탕으로 최적의 성분과 제품을 매칭하여 메인 화면에 노출합니다.

### 2. AR 가상 메이크업 (Real-time AR 체험)

- **face-api.js + tensorflow.js  + SSD MobileNetV1 model**: 얼굴 인식 및 트래킹 기술을 통해 립스틱 등 메이크업 제품을 실시간으로 얼굴에 적용해 봅니다.
